...
function envoieRequeteFormation(){
    ...
    xmlhttp.open("GET", "get_formation.php?q=" + document.getElementById("formation").value, true);
    xmlhttp.send();
	sleep(100);
	...
	$.get('resultat_formation.txt', function(result) {
		if (result == 'ON') {
			...
			// $('#checkbox_competence').append("<br><legend>Compétence</legend>");
			for(var line = 0; line < lines.length; line++){
				if (lines[line] != ""){
				$('#checkbox_competence').append('<input type="checkbox" name="competence[]" value="'+lines[line]+'" />'+lines[line]+'<br>');
				}
			}
		}
	});
